...
export function runContainers(dockerSetupInfo) {
  ...
  const backendStart = runContainer(
    backendContainerName,
    `-p ${dockerSetupInfo.backendPort}:5000 ${dockerConfig.BACKEND_IMAGE_NAME}`,
  );

  const frontendContainerName = getFrontendContainerName(dockerSetupInfo);
  const frontendStart = runContainer(
    frontendContainerName,
    `-p ${dockerSetupInfo.frontendPort}:3000 --env API_URL=http://localhost:${dockerSetupInfo.backendPort} ${dockerConfig.FRONTEND_IMAGE_NAME}`,
  );
